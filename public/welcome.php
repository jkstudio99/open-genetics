...
<head>
  ...
  <link
    href="https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700;800&family=Noto+Sans+Thai:wght@300;400;500;600;700&family=JetBrains+Mono:wght@400;500&display=swap"
    rel="stylesheet">
  ...
  <style>
    ...
    *::after {
    ...
    .orb-1 {
      width: 700px;
      height: 700px;
      background: radial-gradient(circle, rgba(30, 24, 207, .22) 0%, transparent 65%);
      top: -250px;
      left: -180px;
      filter: blur(80px)
    }
    ...
    .orb-2 {
      width: 600px;
      height: 600px;
      background: radial-gradient(circle, rgba(0, 207, 255, .12) 0%, transparent 65%);
      top: 60px;
      right: -180px;
      filter: blur(90px)
    }
    ...
    @keyframes pulseDot {
      ...
      0%,
      100% {
        box-shadow: 0 0 6px #00cfff, 0 0 14px rgba(34, 211, 238, .4)
      }

      50% {
        box-shadow: 0 0 12px #00cfff, 0 0 28px rgba(34, 211, 238, .6)
      }
    }
    ...
    .hero-circle {
      ...
      opacity: .06;
      animation: spin-circle linear infinite
    }
    ...
    [data-theme="light"] .border-indigo-500\/15 {
    ...
    [data-theme="light"] .border-indigo-400\/\[0\.07\] {
      border-color: rgba(79, 70, 229, .08) !important
    }

    [data-theme="light"] .border-cyan-400\/\[0\.05\] {
      border-color: rgba(8, 145, 178, .06) !important
    }
...
<body>
  ...
  <div class="fixed top-0 right-0 z-50 flex items-center gap-1 p-3">
    <button id="theme-toggle"
      class="flex items-center justify-center w-8 h-8 rounded-lg text-white/40 hover:text-white hover:bg-white/[0.06] transition-all"
      title="Toggle theme">
      <svg id="theme-icon-moon" xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="none"
        viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
        <path stroke-linecap="round" stroke-linejoin="round"
          d="M21.752 15.002A9.72 9.72 0 0 1 18 15.75c-5.385 0-9.75-4.365-9.75-9.75 0-1.33.266-2.597.748-3.752A9.753 9.753 0 0 0 3 11.25C3 16.635 7.365 21 12.75 21a9.753 9.753 0 0 0 9.002-5.998Z" />
      </svg>
      <svg id="theme-icon-sun" xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="none" viewBox="0 0 24 24"
        stroke-width="2" stroke="currentColor" class="hidden">
        <path stroke-linecap="round" stroke-linejoin="round"
  ...
  <!-- HERO -->
  <section
    class="relative z-10 min-h-screen flex items-center justify-center text-center px-6 pt-32 pb-24 overflow-hidden">
    <!-- Decorative spinning circles -->
    <div class="absolute inset-0 flex items-center justify-center pointer-events-none" aria-hidden="true">
      <div class="hero-circle w-[36rem] h-[36rem] border-l border-indigo-500/15 rotate-12"
        style="animation-duration:24s"></div>
      <div class="hero-circle w-[52rem] h-[52rem] border-t border-l border-cyan-500/10 rotate-45"
        style="animation-duration:30s"></div>
      <div class="hero-circle w-[68rem] h-[68rem] border-l border-indigo-400/[0.07]"
        style="animation-duration:24s;animation-direction:reverse"></div>
      <div class="hero-circle w-[84rem] h-[84rem] border-r border-cyan-400/[0.05] rotate-45"
        style="animation-duration:36s"></div>
    </div>

    <div class="relative z-20 max-w-[820px] mx-auto">
      <div class="inline-flex items-center gap-2 px-5 py-1.5 rounded-full text-[13px] font-medium text-indigo-300 mb-9"
        style="background:rgba(108,99,255,.08);border:1px solid rgba(108,99,255,.2);backdrop-filter:blur(8px)">
        <span class="w-1.5 h-1.5 rounded-full bg-[#00cfff] dot-pulse"></span>
        <span data-i18n="badge">เฟรมเวิร์คพร้อมใช้งาน</span>
      </div>

      <img id="hero-logo" src="/open-genetics/public/images/logo/open-genetics-logo-white.svg" alt="OpenGenetics"
        class="mx-auto mb-8 block w-auto float-anim"
        style="height:clamp(56px,7vw,84px);filter:drop-shadow(0 0 28px rgba(0,207,255,.35)) drop-shadow(0 0 60px rgba(108,99,255,.2))">

      <h1 class="font-extrabold tracking-[-0.03em] text-white mb-5 leading-[1.1] px-2 sm:px-0"
        style="font-size:clamp(32px,8vw,72px)">
        PHP <span class="grad-text">Micro-Framework</span>
      </h1>

      <p class="text-white/40 max-w-[580px] w-full mx-auto px-4 sm:px-0 mb-10 leading-[1.8] text-[15px] sm:text-[16px] lg:text-[18px]"
        data-i18n-html="subtitle">
        เฟรมเวิร์ค PHP ที่มาพร้อม <strong class="text-white/70 font-medium">JWT Auth</strong>, <strong
          class="text-white/70 font-medium">RBAC</strong>, <strong class="text-white/70 font-medium">i18n</strong> และ
        <strong class="text-white/70 font-medium">Genetic SDK</strong> ให้คุณสร้าง REST API ที่ปลอดภัย เร็ว และขยายได้
      </p>

      <div class="flex flex-col sm:flex-row gap-3 items-center justify-center mb-12">
        <a href="https://open-genetics.vercel.app"
          class="text-white font-semibold text-base sm:text-lg bg-indigo-500 px-7 py-3 sm:py-2.5 rounded-full shadow-lg shadow-indigo-500/30 hover:shadow-xl hover:shadow-indigo-500/40 hover:scale-105 transition-all w-full sm:w-auto text-center"
          data-i18n="btn.docs">
          เอกสาร
        </a>
        <a href="https://github.com/jkstudio99/open-genetics"
          class="text-white/70 font-medium text-base sm:text-lg bg-white/5 px-7 py-3 sm:py-2.5 rounded-full border border-white/10 hover:bg-white/10 hover:border-white/20 transition-all w-full sm:w-auto text-center">
          GitHub
        </a>
      </div>

      <div class="flex items-center gap-1 bg-indigo-500/20 rounded-full pl-5 pr-1.5 py-1.5 max-w-full mx-auto"
        style="width:fit-content">
        <code
          class="text-indigo-300 font-mono font-medium tracking-tight text-[11px] sm:text-sm">composer create-project open-genetics/framework my-api</code>
        <button onclick="copyText('composer create-project open-genetics/framework my-api',this)"
          class="p-2 sm:p-2.5 rounded-full text-indigo-400/60 hover:text-indigo-400 hover:bg-indigo-500/30 transition-all"
          title="Copy">
  ...
  <section class="relative z-10 px-6 pb-24">
    <div class="max-w-[820px] mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
      <div class="p-6 rounded-2xl transition-all"
        style="background:rgba(255,255,255,.025);border:1px solid rgba(255,255,255,.06);backdrop-filter:blur(8px)">
        <div class="w-10 h-10 rounded-xl flex items-center justify-center mb-4"
          ...
          <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="none" viewBox="0 0 24 24"
            stroke-width="1.5" stroke="currentColor" class="text-indigo-400">
            <path stroke-linecap="round" stroke-linejoin="round"
              d="M6.75 7.5l3 2.25-3 2.25m4.5 0h3m-9 8.25h13.5A2.25 2.25 0 0021 18V6a2.25 2.25 0 00-2.25-2.25H5.25A2.25 2.25 0 003 6v12a2.25 2.25 0 002.25 2.25z" />
          </svg>
        </div>
        <h3 class="text-white font-semibold text-base mb-2" data-i18n="f.cli.t">Genetics CLI</h3>
        <p class="text-white/40 text-sm leading-relaxed" data-i18n-html="f.cli.d">ใช้คำสั่ง <code
            class="text-indigo-300 bg-indigo-500/10 px-1.5 py-0.5 rounded text-xs">php genetics help</code> เพื่อสร้าง
          controller, model และ middleware</p>
      </div>
      ...
      <div class="p-6 rounded-2xl transition-all"
        ...
        <p class="text-white/40 text-sm leading-relaxed" data-i18n-html="f.route.d">วางไฟล์ใน <code
            class="text-indigo-300 bg-indigo-500/10 px-1.5 py-0.5 rounded text-xs">api/</code> ระบบจะลงทะเบียน route
          ให้อัตโนมัติ</p>
      </div>
      ...
      <div class="p-6 rounded-2xl transition-all"
        ...
        <p class="text-white/40 text-sm leading-relaxed" data-i18n="f.mw.d">เชื่อมต่อ middleware สำหรับ auth, CORS,
          rate-limit และอื่นๆ</p>
      </div>
      ...
      <div class="p-6 rounded-2xl transition-all"
        ...
        <p class="text-white/40 text-sm leading-relaxed" data-i18n="f.db.d">คิวรีฐานข้อมูลแบบ fluent พร้อม field
          selection และ relationship loading</p>
      </div>
      ...
      <div class="p-6 rounded-2xl transition-all"
        ...
        <p class="text-white/40 text-sm leading-relaxed" data-i18n="f.sec.d">ป้องกัน CSRF, กรองข้อมูล input และ JWT
          authentication พร้อมใช้งาน</p>
      </div>
      ...
      <div class="p-6 rounded-2xl transition-all"
        ...
        <p class="text-white/40 text-sm leading-relaxed" data-i18n="f.zero.d">Convention over configuration
          เริ่มสร้างได้ทันทีด้วยค่าเริ่มต้นที่เหมาะสม</p>
      </div>
    </div>
  </section>
  ...
  <section class="relative z-10 px-6 pb-24">
    <div class="max-w-[820px] mx-auto flex flex-wrap gap-3 justify-center">
      ...
      <div class="inline-flex items-center gap-2 px-4 py-2 rounded-full text-sm font-mono"
        ...
        <span
          class="text-white/40"><?= isset($_SERVER['APP_ENV']) ? htmlspecialchars($_SERVER['APP_ENV']) : 'local' ?></span>
      </div>
    </div>
    <p class="text-center text-white/20 text-sm mt-12" data-i18n="footer">แอปพลิเคชันของคุณพร้อมแล้ว
      เริ่มสร้างสิ่งที่ยอดเยี่ยมกันเถอะ</p>
  </section>
